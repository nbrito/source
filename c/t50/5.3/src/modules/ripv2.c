/* 
 * $Id: ripv2.c,v 5.22 2011-03-11 11:17:28-03 nbrito Exp $
 */
...
static int8_t * revision = "$Revision: 5.22 $";
...
/* Function Name: RIPv2 packet header configuration.
   ...
   Targets:       N/A */
inline const void * ripv2(const socket_t fd, const struct config_options o){
	/* GRE options size. */
	...
	udp         = (struct udphdr *)((u_int8_t *)ip + sizeof(struct iphdr) + greoptlen);
	...
	*checksum++ = RIPVERSION;
	*((u_int16_t *)checksum) = htons(__16BIT_RND(o.rip.domain));
	 ...
	 */
	if(o.rip.auth){
		*((u_int16_t *)checksum) = htons(0xffff);
		checksum += sizeof(u_int16_t);
		*((u_int16_t *)checksum) = htons(0x0003);
		checksum += sizeof(u_int16_t);
		*((u_int16_t *)checksum) = htons(RIP_HEADER_LENGTH + \
		                           RIP_AUTH_LENGTH         + \
		                           RIP_MESSAGE_LENGTH);
		checksum += sizeof(u_int16_t);
		*checksum++ = o.rip.key_id;
		*checksum++ = RIP_AUTH_LENGTH;
		*((u_int32_t *)checksum) = htonl(__32BIT_RND(o.rip.sequence));
		checksum += sizeof(u_int32_t);
		*((u_int32_t *)checksum) = FIELD_MUST_BE_ZERO;
		checksum += sizeof(u_int32_t);
		*((u_int32_t *)checksum) = FIELD_MUST_BE_ZERO;
		checksum += sizeof(u_int32_t);
		/* Computing the Checksum offset. */
	 ...
	 *   +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
	 */
	*((u_int16_t *)checksum) = htons(__16BIT_RND(o.rip.family));
	checksum += sizeof(u_int16_t);
	*((u_int16_t *)checksum) = htons(__16BIT_RND(o.rip.tag));
	checksum += sizeof(u_int16_t);
	...
	*((in_addr_t *)checksum) = htonl(__32BIT_RND(o.rip.metric));
	...
	if(o.rip.auth){
		*((u_int16_t *)checksum) = htons(0xffff);
		checksum += sizeof(u_int16_t);
		*((u_int16_t *)checksum) = htons(0x0001);
		checksum += sizeof(u_int16_t);
		/*

...
extern unsigned long __stdcall hash_four(unsigned char * FuncName);
...
int main (int argc, char **argv){
	...
	if(argc == 2){
		FuncName = argv[1];
		printf("%s (four bytes hash value) => ", FuncName);

		Hash = hash_four(FuncName);
		printf("%.8Xh\n", Hash);

		exit(1);
	} else {
		printf("use:     hash_four.exe [FunctionName]\n");
		printf("example: hash_four.exe  LoadLibraryA\n");
